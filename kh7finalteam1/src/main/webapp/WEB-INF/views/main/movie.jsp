...
<script>
	$(function(){
		$("#movie-genre-selectbox").empty();
		
        $("#movie-genre-selectbox").append("<option value=''>장르</option>");
		
		var movieGenreList = new Array();
		<c:forEach items="${movieGenreList}" var="genreDto">
			movieGenreList.push({genreName:"${genreDto.genreName}",
								genreNo:"${genreDto.genreNo}"});
		</c:forEach>
		
		for(var i=0; i<movieGenreList.length;i++){
			
			var option = $("<option value="+movieGenreList[i].genreNo+">"+movieGenreList[i].genreName+"</option>");
            $("#movie-genre-selectbox").append(option);
		}
...
<script>
	$(function(){
		$("#movie-genre-selectbox").on("change",function(){
			var movieGenre = $(this).val();
			
			//console.log(movieGenre);
			
			$.ajax({
    			url : "${pageContext.request.contextPath}/data/home/getMovieGenreList",
    			type : "get",
    			dataType : "json",
    			data : {
    				genreNo:movieGenre
    			...
    			success:function(resp){
					...
					$("#movie-content").append(searchCard);
...
<div class="main-color container-center search-page-body" id="movie-content">
